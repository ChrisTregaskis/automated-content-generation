...
1. **Receives draft file path** — via manual trigger with test data (webhook trigger for production)
2. **Reads draft JSON** — loads the draft file from `/data/shared/output/drafts/`
3. **Parses draft content** — extracts all fields needed for the Slack message
4. **Looks up image URL** — maps local image paths to public URLs for display
5. **Builds Slack Block Kit message** — creates a rich formatted message with image, content sections, metadata, and action buttons
6. **Posts to Slack** — sends message to `#content-review` channel via HTTP Request
7. **Outputs confirmation** — captures message timestamp for use by Workflow 5
...
│  Draft ID: draft-20260105T163210-10sugc     │
├─────────────────────────────────────────────┤
│  [═══════ IMAGE PREVIEW ═══════]            │
...
2. Slack app created at api.slack.com with:
   ...
   - Interactivity enabled (for button clicks — used by Workflow 5)
...
The native n8n Slack node with "Block Kit" message type doesn't reliably render blocks — only the fallback text appears. Using HTTP Request to call `chat.postMessage` directly ensures proper Block Kit rendering.
...
| ------------------ | ----------------- | ------- |
| ✅ Approve         | `approve_content` | draftId |
| ❌ Reject          | `reject_content`  | draftId |
| ✏️ Request Changes | `request_changes` | draftId |
...
- Slack may cache images — try a different image URL to test
...
- **Workflow 5: Approval Handler** — Webhook to receive button clicks and process approvals
- **Update Workflow 2** — Include image URL in content package for future drafts
